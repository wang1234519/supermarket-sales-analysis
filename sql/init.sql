-- ============================================================
-- 超市销售分析 - 本地 MySQL 建库建表脚本（手工执行用）
-- 在 MySQL 客户端 / Navicat / DataGrip 中执行本文件即可完成建库建表。
-- 说明：项目启动时也会自动执行 resources/schema.sql（内容一致），
--       通常不需要手工执行本脚本。
-- ============================================================

CREATE DATABASE IF NOT EXISTS `supermarket_sales` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- ============================================================
-- 使用流程（完整链路）：
--   1. 虚拟机 Hive：执行 hive/hive-analysis.sql
--      （对原始数据做清洗、计算，产出上面 16 张结果表）
--   2. 虚拟机中执行 hive/sqoop-export.sh
--      （将 16 张结果表从 Hive 导出到本地 MySQL 的 supermarket_sales 库）
--   3. 本地启动本项目（SpringBoot 只读取结果表，展示图表）
-- 登录账号：admin / admin123（首次启动自动创建）
-- ============================================================
